...
UInt32 stream_callback(float *buffers, UInt32 numChannels, UInt32 length, void *userdata)
{
    ...
    assert(numChannels == 2);
    {
        ...
        if (to_copy_samples > samples_in_buf)
            to_copy_samples = samples_in_buf;
...
void keyboardCallback(GLFWwindow *window, int keycode, int scancode, int action, int mods)
{
    ...
    if (g_stream)
        sendUS1UI3(g_stream, PACKETTYPE_C2S_KEYBOARD, keycode, action, calcModkeyBits(mod_shift, mod_ctrl, mod_alt));
}
void mouseButtonCallback(GLFWwindow *window, int button, int action, int mods)
{
    ...
    int mod_alt = mods & GLFW_MOD_ALT;
    if (g_stream)
        sendUS1UI3(g_stream, PACKETTYPE_C2S_MOUSE_BUTTON, button, action, calcModkeyBits(mod_shift, mod_ctrl, mod_alt));
}
...
void on_packet_callback(Stream *s, uint16_t funcid, char *argdata, uint32_t argdatalen)
{
    ...
    if (g_reproxy)
    {
        ...
        case PACKETTYPE_S2C_PROP2D_LOC:
        {
            ...
            if (p)
            {
                ...
                {
                    if (it->second->canSee(p))
                        sendUS1UI3(it->second, PACKETTYPE_S2C_PROP2D_LOC, propid, (int)x, (int)y);
                }
            }
        }
        ...
        case PACKETTYPE_S2C_PROP2D_INDEX_LOC:
        {
            ...
            if (p)
            {
                ...
                {
                    if (it->second->canSee(p))
                        sendUS1UI4(it->second, PACKETTYPE_S2C_PROP2D_INDEX_LOC, propid, index, x, y);
                }
            }
        }
        break; // must go to second step switch.
        case PACKETTYPE_S2C_PROP2D_LOC_VEL:
        {
            ...
            if (p)
            {
                ...
                {
                    if (it->second->canSee(p))
                        sendUS1UI3F2(it->second, PACKETTYPE_S2C_PROP2D_LOC_VEL, id, lx, ly, vx, vy);
                }
            }
        }
        break; // must go to second step switch.
        case PACKETTYPE_S2C_PROP2D_LOC_SCL:
        {
            ...
            if (p)
            {
                ...
                {
                    if (it->second->canSee(p))
                        sendUS1UI3F2(it->second, PACKETTYPE_S2C_PROP2D_LOC_SCL, id, lx, ly, sx, sy);
                }
            }
        }
        break;
    ...
    case PACKETTYPE_S2C_PROP2D_SNAPSHOT:
    {
        ...
        if (pkt.shader_id != 0)
        {
            ...
            if (crs)
            {
                ...
                if (pkt.debug == 1323)
                    print("  colorreplacershader %d found and set", pkt.shader_id, pkt.debug);
            }
            else
            {
                if (pkt.debug == 1323)
    ...
    case PACKETTYPE_S2C_FONT_CREATE:
    {
        ...
        if (g_enable_reprecation)
            f->skip_actual_font_load = true; // don't generate opengl texture when repreproxy
    }
    break;
    ...
    POOL_SCAN(g_prop2d_pool, Prop2D)
    {
        ...
        for (int i = 0; i < p->grid_used_num; i++)
        {
            ...
            if (g->deck)
                ...
                dk_id = p->deck->id;
            if (dk_id)
                sendUS1UI2(newstream, PACKETTYPE_S2C_GRID_DECK, g->id, dk_id);
...
int main(int argc, char **argv)
{
    ...
    if (g_port == HEADLESS_SERVER_PORT && g_enable_reprecation)
        g_port = REPRECATOR_SERVER_PORT;
...
#define CALC_DIFF_PKT(enm) (g_recv_counts[enm] - g_recv_counts_last_second[enm])
    ...
    print("latency average : %f", sum / latency.size());
