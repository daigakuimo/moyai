...
void onConnectCallback(RemoteHead *rh, Client *cl)
{
    ...
    if (g_game_connect_callback)
        g_game_connect_callback(rh, cl);
}
...
void onRemoteKeyboardCallback(Client *cl, int kc, int act, int modshift, int modctrl, int modalt)
{
    ...
    if (g_remote_keyboard_callback)
        g_remote_keyboard_callback(cl, kc, act);
}
void onRemoteMouseButtonCallback(Client *cl, int btn, int act, int modshift, int modctrl, int modalt)
{
    ...
    if (g_remote_mouse_button_callback)
        g_remote_mouse_button_callback(cl, btn, act);
}
...
void sampleCommonInit(int argc, char **argv, const char *title)
{
    ...
    if (headless_mode)
    {
        ...
        char *strPort = argv[2];
        int intPort = 0;
        // if (std::all_of(strPort.cbegin(), strPort.cend(), isdigit))
        // {
        // string の構成文字列が全て数字の場合に true になります。
        intPort = atoi(strPort);
        // }
        if (g_rh->startServer(intPort) == false)
        {
            print("headless server: can't start server. port:%d", intPort);
            exit(1);
        }
        ...
        //     exit(1);
        // }
        if (enable_spritestream)
            g_rh->enableSpriteStream();
        if (enable_videostream)
            g_rh->enableVideoStream(SCRW * RETINA, SCRH * RETINA, 3);
        if (enable_reprecation)
            g_rh->enableReprecation(REPRECATOR_SERVER_PORT);
        if (disable_timestamp)
            g_rh->disableTimestamp();
